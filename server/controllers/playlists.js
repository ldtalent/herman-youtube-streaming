...
class Playlists {
  ...
  async allPlaylists(auth) {
    ...
    const items = res.data.items.map(item => ({id: item.id, ...item.snippet}))
  ...
  async playlistItems(playlistId,auth) {
    ...
    const items = res.data.items.map(item => ({id: item.id, ...item.snippet, videoId: item.snippet.resourceId.videoId}))
    return {...res.data, items};
  }
}
